...
def myfunc():
    try:
        ...
        # sql_data = (6,'신상2',11,5000)  위 아래 같은 코드임
        ...
        cou = cursor.execute(usql, sql_data)   # 리턴 값 받을 수 있다.
        print('수정 건수 : ', cou)
        conn.commit()
        '''
        ...
        # cursor.execute(dsql)
        cou = cursor.execute(dsql)   # 삭제 후 반환값 얻기 (0 또는 1이상)
        if cou != 0:
            print('삭제 성공')
        else:
            print('삭제 실패')
        ...
        print()
        cursor.execute(sql)
        for (a, b, 수량, 단가) in cursor:      # 이 방법도 가능은 하지만 가독성이 떨어짐.
            print(a, b, 수량, 단가)
