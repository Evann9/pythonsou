...
model = LogisticRegression(C=100.0, solver='lbfgs', multi_class='multinomial', random_state=0)
# C: 규제 강도 (L2 규제). 값이 작을수록 규제가 강해짐 (과적합 방지)
# solver: 최적화 알고리즘 선택. 'lbfgs'는 다항 로지스틱 회귀에 권장됨
# multi_class: 다중 분류 방식 설정. 'multinomial'은 소프트맥스 회귀를 사용함
# random_state: 결과 재현성을 위한 난수 시드 설정
